...
import { isKeychainAvailable, saveAllToKeychain } from "./keychain.js";
...
export async function init(options: InitOptions = {}): Promise<void> {
	...
	// 4. Generate wallet (always both EVM + Solana keys)
	...
	// Primary address is based on chosen chain
	const address = isSolana ? solanaAddress : evmAddress;

	const authToken = crypto.randomUUID();
	const port = 18420;

	// 5. Store private keys (keychain or file)
	const config = loadConfig();
	let keyStorage = config.keyStorage;

	if (keyStorage === "keychain") {
		const stored = await saveAllToKeychain(walletName, evmPrivateKey, solanaPrivateKey);
		if (!stored) {
			...
			console.error("  ✔ Stored private keys in system keychain\n");
		}
	}
	...
	fs.writeFileSync(
		envPath,
		buildEnvFile({
			evmPrivateKey: keyStorage === "file" ? evmPrivateKey : undefined,
			solanaPrivateKey: keyStorage === "file" ? solanaPrivateKey : undefined,
			evmAddress,
			solanaAddress,
			chainId,
			chain,
...
function buildEnvFile(opts: EnvFileOptions): string {
	const evmKeyLine = opts.evmPrivateKey ? `PRIVATE_KEY=${opts.evmPrivateKey}\n` : "";
	const solKeyLine = opts.solanaPrivateKey ? `SOLANA_PRIVATE_KEY=${opts.solanaPrivateKey}\n` : "";
	const isSolana = isSolanaChain(opts.chainId);
...
${evmKeyLine}${solKeyLine}WALLET_ADDRESS=${opts.evmAddress}
SOLANA_WALLET_ADDRESS=${opts.solanaAddress}
